---
id: 5-mesmers-city-split
slug: 5-mesmers-city-split
title: 5 Mez City Split
sidebar_label: 5 Mez City Split
---

# 5 Mez City Split

5 Mez City Split remplace l'UA par un MeMo. La run gagne en spiking power, ce qui permet plus de splits, mais perd un peu de survivabilité et de qualité de vie.

:::info Work in progress
Les vidéos gameplay sont manquantes et seront ajoutées bientôt.
:::

## Ce qui change par rapport au 6-0

- MeMo utilise [Healing Seed] là où l'UA utiliserait normalement [Seed of Life].
- [Healing Seed] a une portée beaucoup plus petite que [Seed of Life], donc il doit souvent être placé directement sur le tank.
- VoR est maintenant responsable de placer [Edge of extinction] à peu près aux mêmes endroits que l'UA.
- Foundry room 4 wave 3 et 4 sont tuées plus agressivement, sans attendre que le TT fasse des balls parfaites.
- Le damage mesmer supplémentaire permet à la team de spike les deux inner balls de city en même temps.
- Les deux premières waves de 360 demandent souvent des [con]Scroll of resurrection[/con].

## Rôles

| Rôle | Changement principal | Page |
| --- | --- | --- |
| IAU | Aide à créer le setup rapide de city, spike le côté gauche de city, rejoint Emo pour le gloom rift cap. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/iau) |
| MLK | Spike le côté droit de city, va au 360 avec VoR. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/mlk) |
| TK | Copie [Visions of regret] avec [Arcane Mimicry] pour les spikes côté droit de city et Jadoth. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/tk) |
| VoR | Spike le côté gauche de city, va au 360 avec MLK. Place [Edge of extinction]. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/vor) |
| MeMo | Spike le côté gauche de city. Remplace UA avec plus de spike damage et du support [Healing Seed]. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/memo) |
| Emo | Reçoit le Recall du TT en city et avance pour la transition Jadoth. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/emo) |
| MT | Va faire les deux premieres waves de la 360 avec MLK et VoR. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/mt) |
| TT | Recall Emo en city et transition rapidement vers la Jadoth ball. | [Ouvrir le rôle](/tactics/5-mesmers-city-split/tt) |

## Team build

[bar IAU;OQdTAYB/HaJ4Ew0Z6xmUukmEBCA]

[bar TK;OQdTAYB/HaJ4Ew0Z6JIImUuszBA]

[bar MLK;OQhnAwBhAPEgRYIG0SwJgpz0DdJHwNlGAA]

[bar VoR;OQJTASBbTiu0tZDQwx0Z6JmE6AA]

[bar MeMo;OQNDAqwNS0lLBnAmO4UTP2kSEA]

[bar Emo;OgNDwcb/S1C3MaRkEyDxDHE0lA]

[bar MT;OwFkUld3HPakOzBENJuTDkNJCUnD]

[bar TT;OwFkUld3HPa0BkODNZozEECEuTnD]

## Spiking guide

<a className="sheet-link" href="https://docs.google.com/spreadsheets/d/11slAp8DzHO083dE47c3duhdvnuMnf-VnCzn2jrqOZbg/edit?gid=14503621#gid=14503621" target="_blank" rel="noopener noreferrer">
  <span className="sheet-link__icon" aria-hidden="true"></span>
  <span className="sheet-link__label">Ouvrir la sheet de spike 5 Mez City Split</span>
</a>
